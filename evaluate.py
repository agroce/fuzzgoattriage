...
dcovcache = {}
def dcov(t1, t2):
    if ((t1, t2)) in dcovcache:
        return dcovcache[(t1, t2)]
    if t1 == t2:
        return 0.0
    t1v = cvectors[t1]
    t2v = cvectors[t2]
    ...
    for i in range(0, len(t1v)):
        if (t1v[i] == "1") or (t2v[i] == "1"):
            ...
            if t1v[i] != t2v[i]:
                dc += 1
    dcovcache[(t1, t2)] = (dc/ep)
    dcovcache[(t2, t1)] = (dc/ep)
    return (dc/ep)
...
for K in range(2,39):
    ...
    perfect = 0
    for start in tests:
        ranking = fpf(K, tests, start, dcov)
        bugsSeen = set([])
        for r in ranking:
            for b in bugs[r]:
                bugsSeen.add(b)
    ...
    print "FPF LCOV:", perfect, totalBugCount/len(tests)
